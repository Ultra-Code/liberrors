...
namespace
{
      ...
      try
        {
          ...
          return fmt::format(FMT_COMPILE("{} {} {} {}"), assert_fmt, loc_info,
                             func_proto, backtrace);
        }
      catch (fmt::format_error const & fmt_err)
        {
            constexpr auto loc=std::source_location::current();
            std::cerr<<"Format error in "<<loc.file_name()<<"\n"
                <<"@ line "<<loc.line()<<" : "<<loc.column()<<" of "<<loc.function_name()<<'\n';
  ...
  const auto format_exception = format_error("Exception thrown with message");
